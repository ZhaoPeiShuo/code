...
def plot_graph(add_data,index_evolution,t,historical_data,skip_time,num_skip):
    ...
    model(X_func)
    model_address = "/user/best_model_unet_f.h5"
    ...
    n_s = num_skip
    add_d = add_data
    x, y = load_d_s(add_d, index, time, h_d, s_t, n_s)
...
def cal_error(add_d,time,historical_data,skip_time):
    #Loading the unet model
    model = unet()
    X_func = tf.random.normal([1,128,128,1],mean=0.0, stddev=1.0)
    model(X_func)
    ...
    print("Corresponding foecast error: ", err)
...
if __name__ == "__main__":
    ...
    historical_data = 1
    skip_time = 5
    num_skip = 19        #The number of skips

    # add_d = '/user/test.npz'
    # plot_graph(add_d,index,predicted_time,historical_data,skip_time,num_skip)

    add_d = '/user/test.npz'
    time = [5,10,20,40,60,80,90]
    cal_error(add_d,time,historical_data,skip_time)
